 ...
 * using OpenAI, tailored for Foundry VTT module users (tabletop players).
 *
...
const SYSTEM_PROMPT = `You write release notes for users of Foundry VTT modules. Users are tabletop RPG game masters who install modules into Foundry VTT.

Rules:
- Write 3-8 short bullet points summarizing user-facing changes
- Group related commits into a single bullet
- ONLY mention user-facing changes: new features, bug fixes, UI changes, compatibility updates
- Skip: CI/CD, tooling, dependency bumps, refactors, build changes, test changes, docs
- Be specific (e.g. "Fixed token visibility not updating when switching scenes" not "improved performance")
...
async function generateSummary(notes, logger) {
  ...
  const userPrompt = `Summarize these release notes into user-friendly bullet points for Foundry VTT module users:\n\n${notes}`;
